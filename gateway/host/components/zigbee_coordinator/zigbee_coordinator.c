...
static void configure_rcp_uart(void)
{
    uart_config_t uart_config = {
        .baud_rate = ZIGBEE_COORD_UART_BAUD,
    ...
    ESP_ERROR_CHECK(uart_driver_install(ZIGBEE_COORD_UART_NUM,
                                        2048, 2048, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(ZIGBEE_COORD_UART_NUM, &uart_config));
    ESP_ERROR_CHECK(uart_set_pin(ZIGBEE_COORD_UART_NUM,
                                 ZIGBEE_COORD_UART_TX_PIN,
                                 ZIGBEE_COORD_UART_RX_PIN,
                                 ZIGBEE_COORD_UART_RTS_PIN,
                                 ZIGBEE_COORD_UART_CTS_PIN));

    ESP_LOGI(TAG, "RCP UART configured: baud=%d, TX=%d, RX=%d, RTS=%d, CTS=%d",
             ZIGBEE_COORD_UART_BAUD,
             ZIGBEE_COORD_UART_TX_PIN,
             ZIGBEE_COORD_UART_RX_PIN,
             ZIGBEE_COORD_UART_RTS_PIN,
             ZIGBEE_COORD_UART_CTS_PIN);
